...
Citrus
- displayName: 시트러스
- impression: 밝고 산뜻한 첫인상
- rolePhrase: 무거운 베이스를 환하게 열어주는 역할
- positivePhrase: 조합의 시작을 가볍고 명확하게 만듭니다
- riskPhrase: 베이스가 부족하면 빨리 날아갈 수 있습니다

Woody
- displayName: 우디
- impression: 차분하고 안정적인 깊이감
- rolePhrase: 잔향을 단단하게 잡아주는 역할
- positivePhrase: 전체 조합에 구조와 지속감을 더합니다
- riskPhrase: 과하면 건조하고 무겁게 느껴질 수 있습니다
```
...
초기 구현은 Java `Map` 상수로 시작해도 된다. 다만 문구 수정이 자주 발생할 가능성이 크므로 resource CSV가 더 운영 친화적이다.
...
- 필요하면 `layering_accord_narratives.csv` 추가
- 필요하면 `layering_rule_explanations.csv` 추가
